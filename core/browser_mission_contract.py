...
def required_google_searches(user_text: str) -> int:
    """Count explicit Google-search actions that should end as distinct verified results."""
    text = str(user_text or "")
...
def google_search_result_count(tab_rows: list[dict[str, Any]]) -> int:
    """Count Chrome tabs that currently show a Google search-result URL with a query."""
    count = 0
    for row in tab_rows:
