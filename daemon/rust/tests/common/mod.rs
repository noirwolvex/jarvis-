...
pub struct Pki { pub ca: Certificate, pub server: Certificate, pub server_key: KeyPair, pub client: Certificate, pub client_key: KeyPair }
impl Pki {
    pub fn new() -> Self {
        ...
        let mut server_params = CertificateParams::new(vec!["localhost".into(), "127.0.0.1".into()]).unwrap();
    ...
    pub fn write(&self, directory: &std::path::Path) {
        use std::io::Write;
        for (name, data) in [("ca.pem", self.ca.pem()), ("server.pem", self.server.pem()), ("server-key.pem", self.server_key.serialize_pem()), ("client.pem", self.client.pem()), ("client-key.pem", self.client_key.serialize_pem())] {
            let mut options = std::fs::OpenOptions::new();
            options.write(true).create_new(true);
            #[cfg(unix)]
            { use std::os::unix::fs::OpenOptionsExt; options.mode(0o600); }
            options.open(directory.join(name)).unwrap().write_all(data.as_bytes()).unwrap();
        }
    }
}
