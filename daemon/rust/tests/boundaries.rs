use jarvis_execution_daemon::{capture::{CaptureRing, Frame, ScreenCapture, SimulationCapture}, governance::{Capability, EmergencyLatch, Policy, Scope}, input::{InputController, SimulationInput}, ipc::{Session, MAX_WIRE_BYTES, read_frame}, process::{ProcessManager, validate_argv}, types::{Action, Request, now_ms}};
use std::time::{Duration, Instant};
use tokio::io::AsyncWriteExt;
use uuid::Uuid;

fn request(session: Uuid, action: Action) -> Request { Request { protocol: 1, session, seq: 1, expires_at_ms: now_ms()+1000, request_id: Uuid::new_v4(), capability_id: Some("observe".into()), action } }
fn grant(now: u64) -> Capability { Capability { id: "observe".into(), peer_sha256: "a".repeat(64), expires_at_ms: now+1000, scope: Scope::Observe { display_id: 0 } } }
...
fn forged_wrong_peer_wrong_scope_and_expired_capabilities_fail_closed() {
    ...
    assert!(policy.authorize(&"b".repeat(64), &req, now, &latch).is_err());
    assert!(policy.authorize(&"a".repeat(64), &req, now+1000, &latch).is_err());
    req.action = Action::Capture { display_id: 1 };
    assert!(policy.authorize(&"a".repeat(64), &req, now, &latch).is_err());
    req.action = Action::RunProcess { executable_id: "observe".into(), args: vec![], timeout_ms: 1 };
    assert!(policy.authorize(&"a".repeat(64), &req, now, &latch).is_err());
...
async fn capability_monotonic_expiry_survives_wall_clock_rollback() {
    ...
    assert!(policy.authorize(&"a".repeat(64), &req, now-1000, &EmergencyLatch::default()).is_err());
}
...
async fn emergency_is_shared_idempotent_and_cancels_waiters() {
    ...
    tokio::time::timeout(Duration::from_millis(20), token.cancelled()).await.unwrap();
...
fn stale_frames_out_of_display_coordinates_and_invalid_pixels_are_rejected() {
    ...
    assert!(SimulationInput.click(&frame, 64, 0, &EmergencyLatch::default()).is_err());
    assert!(SimulationInput.click(&frame, -1, 0, &EmergencyLatch::default()).is_err());
    frame.captured_at = Instant::now()-Duration::from_secs(2);
    assert!(SimulationInput.click(&frame, 1, 1, &EmergencyLatch::default()).is_err());
...
fn capture_ring_enforces_count_and_bytes_and_evicts_old_ids() {
    ...
    let first = ring.push(SimulationCapture.capture(0, 16_384).unwrap()).unwrap();
    ring.push(SimulationCapture.capture(0, 16_384).unwrap()).unwrap();
    ring.push(SimulationCapture.capture(0, 16_384).unwrap()).unwrap();
...
fn arbitrary_commands_unknown_json_fields_and_argv_limits_are_rejected() {
    ...
    assert!(manager.validate("rm", &["-rf".into(), "/".into()], 100).is_err());
