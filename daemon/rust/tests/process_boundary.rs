use jarvis_execution_daemon::{
    ...
    process::{ExecutableRule, MAX_OUTPUT_BYTES, ProcessManager, file_sha256},
    types::Reply,
};
...
fn manager(fixture: &str, allow: bool) -> (ProcessManager, Vec<String>) {
    ...
    let rule = ExecutableRule {
        ...
        working_directory: path.parent().unwrap().to_path_buf(),
        path,
        allowed_argv: vec![args.clone()],
    };
...
fn fixture_flood() {
    use std::io::Write;
    let _ = std::io::stdout().write_all(&vec![b'x'; MAX_OUTPUT_BYTES * 4]);
}
...
async fn process_success_streams_both_outputs_and_verifies_exit() {
    ...
    let mut streams = vec![];
    while let Some(Reply::Output { stream, .. }) = rx.recv().await {
        streams.push(stream);
    }
...
async fn process_launch_requires_exact_argv_and_containment_opt_in() {
    ...
    assert!(
        manager
            .validate("fixture", &["--help".into()], 100)
            .is_err()
    );
