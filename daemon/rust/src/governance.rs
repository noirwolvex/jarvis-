use crate::{
    Error, Result,
    types::{Action, Event, Request, now_ms},
};
...
pub enum Scope {
    Observe { display_id: u32 },
    Input { display_id: u32 },
    Process { executable_id: String },
}
...
impl Policy {
    pub fn new(simulation: bool, capabilities: Vec<Capability>, now: u64) -> Result<Self> {
        ...
        for cap in capabilities {
            ...
            let remaining = cap
                .expires_at_ms
                .checked_sub(now)
                .filter(|v| *v > 0 && *v <= MAX_CAPABILITY_TTL_MS)
                .ok_or(Error::Stale)?;
            let expiry = Instant::now() + Duration::from_millis(remaining);
            if grants.insert(cap.id.clone(), (cap, expiry)).is_some() {
                return Err(Error::Denied("duplicate capability"));
            }
        }
        Ok(Self {
            capabilities: grants,
    ...
    pub fn authorize(
        ...
        if matches!(request.action, Action::Status {} | Action::EmergencyStop {}) {
            return Ok(());
        }
        latch.check()?;
        let id = request
            .capability_id
            .as_ref()
            .ok_or(Error::Denied("capability required"))?;
        let (cap, monotonic_expiry) = self
            .capabilities
            .get(id)
            .ok_or(Error::Denied("unknown capability"))?;
        if cap.peer_sha256 != peer {
            return Err(Error::Denied("capability peer mismatch"));
        }
        if cap.expires_at_ms <= now || Instant::now() >= *monotonic_expiry {
            return Err(Error::Stale);
        }
        let requested = match &request.action {
            ...
            Action::Click { display_id, .. } => Scope::Input {
                display_id: *display_id,
            },
        ...
        if cap.scope != requested {
            return Err(Error::Denied("scope mismatch"));
        }
        Ok(())
    }
    ...
    pub fn action_deadline_ms(&self, request: &Request) -> u64 {
        request
            ...
            .map(|(c, _)| c.expires_at_ms.min(request.expires_at_ms))
...
impl EventJournal {
    pub fn new(capacity: usize) -> Result<Self> {
        if capacity == 0 || capacity > 4096 {
            return Err(Error::Limit("event journal"));
        }
        Ok(Self {
