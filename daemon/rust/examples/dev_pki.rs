...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let directory = PathBuf::from(std::env::args_os().nth(1).ok_or("usage: cargo run --example dev_pki -- <NEW private directory>")?);
    // Refuse an existing directory so no key or configuration can be overwritten.
    std::fs::create_dir(&directory)?;
    #[cfg(unix)]
    { use std::os::unix::fs::PermissionsExt; std::fs::set_permissions(&directory, std::fs::Permissions::from_mode(0o700))?; }
    ...
    std::fs::OpenOptions::new().write(true).create_new(true).open(directory.join("config.json"))?.write_all(&serde_json::to_vec_pretty(&config)?)?;
    println!("Development PKI written; simulation grants expire in five minutes. Windows: restrict this directory's ACL to your user before use. Start jarvis-daemon with its config.json.");
    Ok(())
}
