...
export function ControlCenter() {
  ...
  const control = async (action: string) => {
    ...
    try {
      const response = await fetch("/api/control", {
        method: "POST", headers: { "Content-Type": "application/json", "X-Jarvis-Control": "simulation" },
      ...
      await refresh();
      setNotice(action === "run" ? "Simulation queued. Follow each action in the execution timeline." : action === "stop" ? "Simulation stopped. Explicit reset is required before new work." : "Control state updated.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Control request failed."); }
    finally { setBusy(false); }
  };

  const exportEvents = () => {
    const url = URL.createObjectURL(new Blob([JSON.stringify({ mode: "simulation", exportedAt: new Date().toISOString(), events: snapshot.events }, null, 2)], { type: "application/json" }));
    const anchor = document.createElement("a"); anchor.href = url; anchor.download = "jarvis-simulation-events.json"; anchor.click(); URL.revokeObjectURL(url);
  ...
  return <div className={`console ${light ? "light" : ""}`}>
    ...
    <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
      ...
      <div className="workspace-select"><div className="workspace-icon"><Icon name="terminal" size={18} /></div><div>Local workspace<small>Reference environment</small></div><span className="tiny-tag">01</span></div>
      ...
      <div className="sidebar-bottom"><div className="environment"><span className="environment-dot" /><div>Simulation environment<small>Local · No native actions</small></div><Icon name="security" size={16} /></div><button className="profile" onClick={() => navigate("security")}><span className="avatar">OP</span><span>Local operator<small>Workspace administrator</small></span><Icon name="chevron" size={16} /></button></div>
    </aside>
    ...
    <div className="workspace">
      ...
      <main id="main" className="main-content">
        <div className="page-heading"><div><div className="eyebrow"><span className="small-cross">+</span> JARVIS X / CONTROL CENTER</div><h1>{navigation.find(n => n.id === page)?.name}</h1><p>{page === "mission" ? "Intelligence proposes. Evidence confirms. You stay in control." : subtitles[page]}</p></div><div className="heading-actions"><button className="button secondary" disabled={busy || halted || connection !== "connected"} onClick={() => void control(snapshot.status === "PAUSED" ? "resume" : "pause")}><Icon name={snapshot.status === "PAUSED" ? "play" : "pause"} size={14} />{snapshot.status === "PAUSED" ? "Resume" : "Pause"}</button><button className="button stop-button" disabled={busy || halted || connection !== "connected"} onClick={() => void control("stop")}><Icon name="stop" size={13} />Stop simulation</button></div></div>
        ...
        {halted && <div className="banner emergency" role="alert"><Icon name="stop" /><span>Emergency stop is latched. Active simulation work was cancelled.</span><button disabled={busy || connection !== "connected"} onClick={() => void control("reset")}>Reset simulation stop</button></div>}
        ...
        {page === "mission" && <>
          <section className="mission-launch">
            <div className="mission-intro"><span className="overline"><i /> VERIFIED EXECUTION, BY DESIGN</span><h2>A clear objective.<br /><span>A verified outcome.</span></h2><p>Explore a complete mission, from the first observation<br className="desktop-break" /> to the final evidence-backed state.</p><form className="mission-input" onSubmit={e => { e.preventDefault(); void control("run"); }}><Icon name="terminal" size={18} /><input aria-label="Simulation mission title" value={title} maxLength={160} onChange={e => setTitle(e.target.value)} placeholder="Give this simulation a name…" /><button className="button primary" type="submit" disabled={busy || halted || !title.trim() || connection !== "connected"}><Icon name="play" size={13} />Run simulation</button></form><div className="input-caption"><Icon name="security" size={12} /> Fixed workspace-report workflow · In-memory files · No model required</div></div>
          ...
          <div className="metrics-strip"><Metric label="RUNTIME STATE" value={formatStatus(snapshot.status)} detail="Local simulation" icon="resources" /><Metric label="MISSIONS COMPLETED" value={String(completed).padStart(2, "0")} detail={`${snapshot.tasks.length} submitted in this session`} icon="check" /><Metric label="VERIFICATION EVENTS" value={String(verified).padStart(2, "0")} detail="Observed outcome checks" icon="security" /><Metric label="EVENTS RETAINED" value={String(snapshot.events.length).padStart(2, "0")} detail="Bounded, session event window" icon="timeline" /></div>

          <div className="mission-grid"><section className="panel vision-panel"><SectionTitle title="Computer view" subtitle="A view into the simulated workspace" action={<button className="text-button" onClick={() => navigate("vision")}>Inspect <Icon name="external" size={13} /></button>} /><ComputerView snapshot={snapshot} /><div className="vision-footer"><span><i /> SYNTHETIC SCENE</span><span>World version <b>{snapshot.worldVersion}</b></span><span>Source <b>simulation adapter</b></span></div></section><section className="panel activity-panel"><SectionTitle title="Execution feed" action={<span className="small-pill">SESSION</span>} /><EventList events={snapshot.events.slice(-5).reverse()} compact /><button className="panel-link" onClick={() => navigate("timeline")}>Open execution timeline<Icon name="arrow" size={15} /></button></section></div>

          <section className="panel missions-panel"><SectionTitle title="Mission history" subtitle="Every run has an objective, a plan, and a trace." action={<span className="small-pill">{snapshot.tasks.length} TOTAL</span>} /><MissionTable tasks={snapshot.tasks} onInspect={task => { setSelectedNode(task.nodes[0]?.id ?? ""); navigate("graph"); }} /></section>
        </>}

        {page === "vision" && <div className="detail-grid"><section className="panel"><SectionTitle title="World observation" subtitle="Synthetic scene rendered from the simulation state" action={<span className="small-pill">SIMULATION</span>} /><ComputerView snapshot={snapshot} large /><div className="vision-footer"><span>World version <b>{snapshot.worldVersion}</b></span><span>Desktop capture <b>Not connected</b></span></div></section><section className="panel"><SectionTitle title="Observed facts" subtitle="Current structured world state" /><div className="fact-list">{snapshot.facts.length ? snapshot.facts.map(f => <div className="fact" key={f.key}><span>{f.key}</span><code>{f.value}</code></div>) : <Empty icon="vision" title="No observations yet" text="Run the workspace simulation to populate the world model." />}</div><div className="inline-note"><Icon name="info" size={16} /><p>Native screen capture, OCR, and accessibility fusion are separate integration stages. This view displays simulation facts.</p></div></section></div>}
...
function ComputerView({ snapshot, large = false }: { snapshot: Snapshot; large?: boolean }) {
  const documents = snapshot.facts.filter(f => /file|report|content|path/i.test(f.key));
