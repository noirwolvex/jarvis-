...
function ComputerView({ snapshot, large = false }: { snapshot: Snapshot; large?: boolean }) {
  if (snapshot.mode === "hybrid") {
    ...
    return capture ? <div className="computer-view" style={{ padding: 12 }}>
      ...
      <p className="mono" style={{ fontSize: 11 }}>{capture.live ? "Latest screen change" : "Captured"} {new Date(capture.capturedAtMs).toLocaleTimeString()} · {capture.display.width} × {capture.display.height}{snapshot.status !== "EXECUTING" ? " · Mission capture paused" : ""}</p>
    </div> : <Empty icon="vision" title="Awaiting a screen observation" text="Full Access missions update this preview when the desktop changes. Screen monitoring runs only during a mission." />;
  }
